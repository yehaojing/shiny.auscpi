...
shinyServer(function(input, output, session) {
    ...
    data <- data %>%
        filter(!(
            parentID %in% c('115901', '104099', '131198', '104100', '102674')
        ),
        TSEST == '10') %>%
        fix_rounding_issue()
    ...
    observeEvent({
        ...
        if (is.null(c))
            clickData$DataID <- list(DataID = 0)
    ...
    output$bar_plot <- renderPlotly({
        ...
        p1 <- plot_ly(
            ...
            layout(
                title = paste(selected_hierarchy()$indexName),
                margin = list(b = 120),
    ...
    output$line_plot <- renderPlotly({
        d <-
            as.data.frame(d_subplots()) %>% adjust_for_inflation(input$adjusted_quarter)
        ...
        p2 <- plot_ly(
            ...
            layout(
                title = selected_hierarchy()$indexName,
                xaxis = list(
                    title = 'Quarter',
                ...
                shapes = list(
                    type = 'line',
                    x0 = {
                        data_date_lookup %>% filter(obsTime == input$adjusted_quarter) %>% .$quarterEnd
                    },
                    x1 = {
                        data_date_lookup %>% filter(obsTime == input$adjusted_quarter) %>% .$quarterEnd
                    },
